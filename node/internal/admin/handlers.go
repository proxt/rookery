// Package admin implements the node's web admin panel: login, per-user
// rookery:// link management, and node settings.
package admin
...
func (s *Server) toView(u store.User, publicAddr string) userView {
	link := profile.Encode(profile.Link{
		NodeAddr: publicAddr,
		UserID:   u.ID,
		Secret:   u.Secret,
		Name:     u.Name,
	})
	return userView{ID: u.ID, Name: u.Name, Link: link, CreatedAt: u.CreatedAt.Format(time.RFC3339)}
}
...
func (s *Server) handleListUsers(w http.ResponseWriter, r *http.Request) {
	...
	for _, u := range list {
		views = append(views, s.toView(u, publicAddr))
	}
	writeJSON(w, views)
}
...
func (s *Server) handleCreateUser(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, s.toView(u, publicAddr))
}
